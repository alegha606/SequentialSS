...
cd "C:\Users\User XYZ\Stata Results Tables_Base Case\"
...
cslope_bscorrected cslope_bscorrected_pctile2dot5 cslope_bscorrected_pctile97dot5 ///
cstat_opt cstat_opt_pctile2dot5 cstat_opt_pctile97dot5 ///
...
/* 1.2 Apply the initial model (can change this later to lasso etc) which is 
the base-case unpenalised model to the development dataset: 
...
/* 1.22 Calculate the C-statistic/AUC to evaluate the apparent discrimination 
performance of the model */

qui pmcstat logpr_original AKI
local cstat_orig = r(cstat)

/* 1.23 Also assessing the models apparent calibration via the 
calibration slope.*/

qui logistic AKI logpr_original, coef
local cslope_orig = _b[logpr_original]

/* 1.3 Save predictions and original data in a new dataset */

save "basecase_originaldevdatawithmodelresults", replace

/* ************************************************************************** */
/* 2. START OF SECOND (FINAL AND INNERMOST) LOOP: 'B' BOOTSTRAP REPS TO:      */
/*                                                                            */
/* i) PERFORM INTERNAL VALIDATION (AND GENERATE OPTIMISM ADJUSTED C-SLOPE ETC)*/
/* ii) AND TO GENERATE B=200 RISK ESTIMATES PER INDIVIDUAL (FOR INSTABILITY   */
/* MEASURES)                                                                  */
/* ************************************************************************** */

/*
	Note: for the internal validation results (overall model performance measures),
	we store these in a matrix (B=200 bootstrap reps, and 2 overall model performance 
	measures: c-slope and c-statistic, both calculated for the bootstrap model applied 
	to bootstrap sample (i.e. 'apparent' performance) as well as for the bootstrap model 
	applied to the original model development dataset from step 1.2 of code 
	(i.e. 'test' performance)
	
	3x measures relating to EVPI are also stored (these only relate to the bootstrap
	model applied to the original development data)
	
	So the results matrix contains 7x performance overall model performance 
	measures in total, and 200 rows)	
	
	Also storing the individual risk estimates, separately in a dataset (not matrix), 
	of the n=`i' observations for each of the B=200 replications. 
	
	Iteratively storing these individual risk estimates into the same dataset 
	created in step 1.3 of code, that contains the original model risk estimates: 
	'basecase_originaldevdatawithmodelresults.dta'.
	
	So we at the end of this bootstrap, we are storing 
	(i) a matrix of overall performance measures, and
	(ii) a dataset of individual risk estimates across the B=200 bootstrap reps.

*/

matrix results = J(200,7,.)
...
use "basecase_originaldevdatawithmodelresults", clear
keep AKI bicarbonate_mean creatinine_mean hemoglobin_mean bun_mean potassium_mean sysbp_mean
...
/* 3.1 Appling the initial model (can change this later to lasso etc) which is 
the base-case model to bootstrap dataset: 
...
/* 3.12 Calculate the C-statistic/AUC to evaluate the apparent discrimination 
performance of the model */

pmcstat lp AKI
matrix results[`k',1] = r(cstat)

/* 3.13 Also assessing the models apparent calibration via the 
calibration slope.*/
...
/* 3.23 Calculate the C-statistic/AUC to evaluate the test discrimination 
performance of the model */

pmcstat logp`k' AKI
matrix results[`k',3] = r(cstat)

/* 3.24 Also assessing the models test calibration by via the calibration slope.*/

logistic AKI logp`k', coef
matrix results[`k',4] = _b[logp`k']

/* 3.25 Now performing intermediary calculations to enable bootstrap-based computation 
of the Expected Value of Perfect Information (EVPI) */

/* 3.251 First estimating 'NB_All' for this current kth bootstrap rep 
...
/* 3.252 Then estimating 'NB_Model' (NB_Model is the net benefit of using the 
...
/* 3.253 Finally estimating 'NB_Max' (NB_Max is the net benefit of using the 
...
/* MEASURES:                                                                  */
/*                                                                            */
/* 1) OPTIMISM IN C-STAT                                                      */
/*                                                                            */
/* 2) OPTIMISM ADJUSTED C-SLOPE                                               */
/*                                                                            */
/* 3) EVPI                                                                    */
/*                                                                            */
/* ************************************************************************** */
...
mat colnames results = cstat_boot cslope_boot cstat_test cslope_test NB_All NB_Model NB_Max
...
of apparent and test performance. So now we can calculate the optimism */

gen optimism_c = cstat_boot - cstat_test
gen optimism_slope = cslope_boot - cslope_test

/* 4.03 Summarise the optimism in the performance
...
/* 4.031 Starting with the optimism in the c-stat */

/* Summarising and storing the mean, 2.5th and 97.5th percentiles */

qui su optimism_c 
local cstat_opt = r(mean)

_pctile optimism_c, p(2.5)
local cstat_opt_pctile2dot5 = r(r1)

_pctile optimism_c, p(97.5)
local cstat_opt_pctile97dot5 = r(r1)

/* 4.032 Also summarising the optimism in calibration slope, this will be needed 
to correct the original c-slope for optimism */
...
_pctile optimism_slope, p(2.5)
local cslope_opt_pctile2dot5 = r(r1)

_pctile optimism_slope, p(97.5)
local cslope_opt_pctile97dot5 = r(r1)


/* 4.04 Now calculating the optimism adjusted performance of the original model by
	subtracting the optimism from the original model's apparent calibration slope, 
	and storing the optimism adjusted c-slope (and corresponding 2.5th and 97.5th 
	percentiles) */
	
local cslope_bscorrected = `cslope_orig' - `cslope_opt'

local cslope_bscorrected_pctile2dot5 = `cslope_orig' - `cslope_opt_pctile2dot5'

local cslope_bscorrected_pctile97dot5 = `cslope_orig' - `cslope_opt_pctile97dot5'
...
use "basecase_originaldevdatawithmodelresults", replace
order *, sequential
...
post `FinalDataset' (`n') (`cslope_bscorrected') (`cslope_bscorrected_pctile2dot5') (`cslope_bscorrected_pctile97dot5') (`cstat_opt') (`cstat_opt_pctile2dot5') (`cstat_opt_pctile97dot5') (`UIwidth_mean') (`UIwidth_pctile2dot5') (`UIwidth_pctile97dot5') (`Delta_mean') (`Delta_pctile2dot5') (`Delta_pctile97dot5') (`MAPE_mean') (`MAPE_pctile2dot5') (`MAPE_pctile97dot5') (`probmisclass_mean') (`probmisclass_pctile2dot5') (`probmisclass_pctile97dot5') (`EVPI')
...
/* 7.1 Learning Curve for: Mean Bootstrap Corrected Calibration Slope */

gen StoppingRuleCriteria = 0.9

twoway line cslope_bscorrected cslope_bscorrected_pctile2dot5 cslope_bscorrected_pctile97dot5 StoppingRuleCriteria n, ///
ytitle(Bootstrap Corrected Calibration Slope) xtitle(n) legend(order(1 "Mean" 2 "2.5th Percentile" 3 "97.5th Percentile" 4 "Stopping Rule Criteria")) ///
lcolor (midblue gray gray black) lpat(solid solid solid dash) lwidth(*4 *1 *1 *1) legend(size(*0.75)) xlabel(0(500)3000) ylabel(0.0(.1)1.50)

/* 7.2 Learning Curve for: Mean Optimism in AUC */

drop StoppingRuleCriteria

gen StoppingRuleCriteria = 0.02	

twoway line cstat_opt cstat_opt_pctile2dot5 cstat_opt_pctile97dot5 StoppingRuleCriteria n, ///
ytitle(Optimism in AUC) xtitle(n) legend(order(1 "Mean" 2 "2.5th Percentile" 3 "97.5th Percentile" 4 "Stopping Rule Criteria")) ///
lcolor (midblue gray gray black) lpat(solid solid solid dash) lwidth(*4 *1 *1 *1) legend(size(*0.75)) xlabel(0(500)3000) ylabel(-0.04(.02)0.24)

/* 7.3 Learning Curve for: Mean UI width */
...
lcolor (midblue gray gray black) lpat(solid solid solid dash) lwidth(*4 *1 *1 *1) legend(size(*0.75)) xlabel(0(500)3000) ylabel(0.0(.1)0.9)

/* 7.4 Learning Curve for: Mean Delta */
...
lcolor (midblue gray gray black) lpat(solid solid solid dash) lwidth(*4 *1 *1 *1) legend(size(*0.75)) xlabel(0(500)3000) ylabel(0.0(.05)0.6)

/* 7.5 Learning Curve for: Mean MAPE */
...
lcolor (midblue gray gray black) lpat(solid solid solid dash) lwidth(*4 *1 *1 *1) legend(size(*0.75)) xlabel(0(500)3000) ylabel(0.0(.02)0.18)				

/* 7.6 Learning Curve for: Mean Probability of Misclassification (10% threshold) */
...
lcolor (midblue gray gray black) lpat(solid solid solid dash) lwidth(*4 *1 *1 *1) legend(size(*0.75)) xlabel(0(500)3000) 

/* 7.7 Learning Curve for: Mean EVPI */
...
lcolor (midblue black) lpat(solid dash) lwidth(*4 *1) legend(size(*0.75)) xlabel(0(500)3000) ylabel(0.0(.001)0.012)
...
/* Optimism Adjusted Calibration Slope >= 0.9                                 */
/* ************************************************************************** */
...
gen NextSSMeasureValue = cslope_bscorrected[_n+1]
gen ThirdConsecutiveSSMeasureValue = cslope_bscorrected[_n+2]
gen FourthConsecutiveSSMeasureValue = cslope_bscorrected[_n+3]
gen FifthConsecutiveSSMeasureValue = cslope_bscorrected[_n+4]
...
replace StoppingRuleMetTwice = 1 if ((cslope_bscorrected >= 0.9) & (NextSSMeasureValue >= 0.9))

summ n if StoppingRuleMetTwice == 1

di as err "Minimum sequential SS based on stopping rule of a bootstrap-corrected calibration slope of >=0.9 and two consecutive occurrences is: " r(min)
...
replace StoppingRuleMetThreeTimes = 1 if ((cslope_bscorrected >= 0.9) & (NextSSMeasureValue >= 0.9) & (ThirdConsecutiveSSMeasureValue >= 0.9))

summ n if StoppingRuleMetThreeTimes == 1

di as err "Minimum sequential SS based on stopping rule of a bootstrap-corrected calibration slope of >=0.9 and three consecutive occurrences is: " r(min)
...
replace StoppingRuleMetFiveTimes = 1 if ((cslope_bscorrected >= 0.9) & (NextSSMeasureValue >= 0.9) & (ThirdConsecutiveSSMeasureValue >= 0.9) & (FourthConsecutiveSSMeasureValue >= 0.9) & (FifthConsecutiveSSMeasureValue >= 0.9))

summ n if StoppingRuleMetFiveTimes == 1

di as err "Minimum sequential SS based on stopping rule of a bootstrap-corrected calibration slope of >=0.9 and five consecutive occurrences is: " r(min)


/* ************************************************************************** */
/* 8.2 Minimum Required Sample Size Based on Stopping Rule Criteria of:       */
/* Mean AUC Optimism <= 0.02                                                  */
/* ************************************************************************** */
...
gen NextSSMeasureValue = cstat_opt[_n+1]
gen ThirdConsecutiveSSMeasureValue = cstat_opt[_n+2]
gen FourthConsecutiveSSMeasureValue = cstat_opt[_n+3]
gen FifthConsecutiveSSMeasureValue = cstat_opt[_n+4]
...
gen StoppingRuleMetTwice = 0 
replace StoppingRuleMetTwice = 1 if ((cstat_opt <= 0.02) & (NextSSMeasureValue <= 0.02))

summ n if StoppingRuleMetTwice == 1

di as err "Minimum sequential SS based on stopping rule of mean AUC optimism <=0.02 and two consecutive occurrences is: " r(min)
...
gen StoppingRuleMetThreeTimes = 0 
replace StoppingRuleMetThreeTimes = 1 if ((cstat_opt <= 0.02) & (NextSSMeasureValue <= 0.02) & (ThirdConsecutiveSSMeasureValue <= 0.02))

summ n if StoppingRuleMetThreeTimes == 1

di as err "Minimum sequential SS based on stopping rule of mean AUC optimism <=0.02 and three consecutive occurrences is: " r(min)
...
gen StoppingRuleMetFiveTimes = 0 
replace StoppingRuleMetFiveTimes = 1 if ((cstat_opt <= 0.02) & (NextSSMeasureValue <= 0.02) & (ThirdConsecutiveSSMeasureValue <= 0.02) & (FourthConsecutiveSSMeasureValue <= 0.02) & (FifthConsecutiveSSMeasureValue <= 0.02))

summ n if StoppingRuleMetFiveTimes == 1

di as err "Minimum sequential SS based on stopping rule of mean AUC optimism <=0.02 and five consecutive occurrences is: " r(min)
...
/* ************************************************************************** */
/* 8.6 Minimum Required Sample Size Based on Stopping Rule Criteria of:       */
/* Mean Probability of Misclassification <=0.1                                */
/* ************************************************************************** */

/* 8.61 First open the final learning curve dataset, and just keep the variables 
for 'n' (sample size increment) and the performance measure of interest */
...
/* 8.62 Re-sort data so that n goes from smallest to largest */

sort n

/* 8.63 Now assess the performance measure value for the next sample size increment 
(i.e. n = current SS + 100),
so can check if stopping rule criteria sustained over two consecutive SS increments
(or further increments as necessary, here I'm checking for 2, 3 and 5, 
consecutive increments, respectively), as this is what I require for each 
...
/* 8.64 Create a variable to indicate whether the particular stopping rule criteria
has been met over two consecutive sample size increments or not, and if so, 
when is the minimum sample size that this occurs */
...
/* 8.65 Create a variable to indicate whether the particular stopping rule criteria
has been met over three consecutive sample size increments or not, and if so, 
when is the minimum sample size that this occurs */
...
/* 8.66 Create a variable to indicate whether the particular stopping rule criteria
has been met over five consecutive sample size increments or not, and if so, 
when is the minimum sample size that this occurs */
...
/* ************************************************************************** */
/* 8.7 Minimum Required Sample Size Based on Stopping Rule Criteria of:       */
/* Mean EVPI <=0.001                                                          */
/* ************************************************************************** */

/* 8.71 First open the final learning curve dataset, and just keep the variables 
for 'n' (sample size increment) and the performance measure of interest */
...
/* 8.72 Re-sort data so that n goes from smallest to largest */

sort n

/* 8.73 Now assess the performance measure value for the next sample size increment 
(i.e. n = current SS + 100),
so can check if stopping rule criteria sustained over two consecutive SS increments
(or further increments as necessary, here I'm checking for 2, 3 and 5, 
consecutive increments, respectively), as this is what I require for each 
...
/* 8.74 Create a variable to indicate whether the particular stopping rule criteria
has been met over two consecutive sample size increments or not, and if so, 
when is the minimum sample size that this occurs */
...
/* 8.75 Create a variable to indicate whether the particular stopping rule criteria
has been met over three consecutive sample size increments or not, and if so, 
when is the minimum sample size that this occurs */
...
/* 8.76 Create a variable to indicate whether the particular stopping rule criteria
has been met over five consecutive sample size increments or not, and if so, 
when is the minimum sample size that this occurs */
