...
cd "C:\Users\User XYZ\Stata Results Tables_Unpenalised Log Reg + Heuristic Uniform Shrinkage\"
...
/* 1.2 Apply the initial model (this will be unpenalised logistic regression 
first, then heuristic uniform shrinkage applied to this model): 
We are first fitting a logistic regression model to determine the probability of 
...
/* 1.21 Calculate the predicted probabilities (let's name them 'pr_noshrinkage') 
...
/* 1.3 Now we apply Van Houwelingen's heuristic shrinkage to the original model */ 

/* 1.31 First need to fit the base-case (unpenalised logistic regression) model 
again so that it is in Stata's memory */

qui logistic AKI bicarbonate_mean creatinine_mean hemoglobin_mean ///
bun_mean potassium_mean sysbp_mean, iterate(500)

/* 1.32 Now we store the constant/intercept term to update later post shrinkage */

global orig_cons = _b[_cons]

/* 1.33 Then we calculate and store the heuristic shrinkage of Van Houwelingen 
using the formula as defined in Supplementary Material S3 */
 
local h_shrink = (e(chi2) - e(df_m)) / e(chi2)

/* 1.34 Then we apply heuristic shrinkage to our final model by multiplying the 
coefficients of the model (the "betas") by this shrinkage value.
 However the linear predictor of a logistic model includes the constant/intercept 
 term which must be re-estimated post shrinkage of the betas.
 
 Therefore we must first remove this element from the linear predictor variable 
 and then multiply the betas by the shrinkage factor */
 
gen lp_nocons = lp_noshrinkage - $orig_cons
gen heuristic_lp_nocons = lp_nocons*`h_shrink'	

/* 1.35 Now to re-estimate a new intercept value allowing for the shrunken betas
 we fit a logistic regression model with the shrunken linear predictor set as 
 an offset to 'fix' these shrunken beta coefficients. We then add this 
 additional intercept term to the linear predictor to obtain our final shrunken 
 model linear predictor */
 
qui logistic AKI, offset(heuristic_lp_nocons) coef
global new_cons = _b[_cons]

gen heuristic_lp = heuristic_lp_nocons + $new_cons

/* 1.36 Then store the predicted probabilities of development of acute kidney 
injury */

gen pr_original = exp(heuristic_lp)/(1+exp(heuristic_lp))

/* 1.4 Save predictions and original data in a new dataset */

save "HS_originaldevdatawithmodelresults", replace
...
use "HS_originaldevdatawithmodelresults", clear
keep AKI bicarbonate_mean creatinine_mean hemoglobin_mean bun_mean potassium_mean sysbp_mean
...
/* 2.1 Apply the initial model to the bootstrap dataset 
(this will be unpenalised logistic regression first, then heuristic uniform 
shrinkage applied to this model): 
We are first fitting a logistic regression model to determine the probability of 
developing AKI (outcome) based on 6 (continuous) predictors 
(linear predictor-outcome relationships are assumed, and all 6 variables 
are forced into the model). */
...
/* 2.11 Calculate the predicted probabilities (let's name them 'pr_noshrinkage') 
and the value of the linear predictor (i.e. 'pr_noshrinkage' but on logit-p scale 
rather than p-scale, let's name: 'lp_noshrinkage') for each individual using 
the predict command. */
...
/* 2.2 Now we apply Van Houwelingen's heuristic shrinkage to the original bootstrap 
model */ 

/* 2.21 First need to fit the base-case (unpenalised logistic regression) model 
again so that it is in Stata's memory */

qui logistic AKI bicarbonate_mean creatinine_mean hemoglobin_mean ///
bun_mean potassium_mean sysbp_mean, iterate(500)

/* 2.22 Now we store the constant/intercept term to update later post shrinkage */

global orig_cons = _b[_cons]

/* 2.23 Then we calculate and store the heuristic shrinkage of Van Houwelingen 
using the formula as defined in Supplementary Material S3 */
 
global h_shrink_bs = (e(chi2) - e(df_m)) / e(chi2)

/* 2.24 Then we apply heuristic shrinkage to our final model by multiplying the 
coefficients of the model (the "betas") by this shrinkage value.
 However the linear predictor of a logistic model includes the constant/intercept 
 term which must be re-estimated post shrinkage of the betas.
...
gen lp_nocons = lp_noshrinkage - $orig_cons
gen heuristic_lp_nocons = lp_nocons*$h_shrink_bs

/* 2.25 Now to re-estimate a new intercept value allowing for the shrunken betas
 we fit a logistic regression model with the shrunken linear predictor set as 
 an offset to 'fix' these shrunken beta coefficients. We then add this 
 additional intercept term to the linear predictor to obtain our final shrunken 
 model linear predictor */
...
/* 2.26 Now we have the shrinkage term h_shrink for the kth bootstrap rep,
and the constant term new_cons`k' (adjusted after shrinkage applied to beta coeffs) 
for the kth bootstrap rep...

So we have everything we need to be able to calculate the shrunken coeffs (betas),
first we just need to re-run the model again so it's in Stata' memory to be able 
to extract original unshrunken betas, then we apply shrinkage to them */

logistic AKI bicarbonate_mean creatinine_mean hemoglobin_mean ///
bun_mean potassium_mean sysbp_mean, iterate(500)

/* 2.27 Finally, we can calculate and store the heuristic shrunken beta terms for 
each individual predictor from the heuristic shrinkage model. We will need 
these to be able to apply the heuristic shrinkage model generated from the 
bootstrap sample onto the original development data in the next step.

This step is needed as we don't have the model already stored in Stata memory,
so one solution is to manually construct it. */

global bicarbonate_mean = _b[bicarbonate_mean]*$h_shrink_bs
global creatinine_mean = _b[creatinine_mean]*$h_shrink_bs
global hemoglobin_mean = _b[hemoglobin_mean]*$h_shrink_bs
global bun_mean = _b[bun_mean]*$h_shrink_bs
global potassium_mean = _b[potassium_mean]*$h_shrink_bs
global sysbp_mean = _b[sysbp_mean]*$h_shrink_bs
...
/* 3.2 Now testing performance of the heuristic shrinkage bootstrap model in 
	the original sample */
...
/* 3.22 Predict probabilities (manually now) & linear predictor from the 
bootstrap heuristic shrinkage model applied to the original dataset and save, 
alongside original model predictions.
...
replace logp`k' = $new_cons + bicarbonate_mean*$bicarbonate_mean ///
+ creatinine_mean*$creatinine_mean + hemoglobin_mean*$hemoglobin_mean ///
+ bun_mean*$bun_mean + potassium_mean*$potassium_mean + sysbp_mean*$sysbp_mean 

gen pr`k' = exp(logp`k')/(1+exp(logp`k'))
...
/* ************************************************************************** */
/* 4. USING BOOTSTRAP RESULTS TO CALCULATE EVPI                               */
/*                                                                            */
/* ************************************************************************** */
...
/* 4.11 First calculating the ENB_All, ENB_Model, and ENB_Max by taking the 
...
use "HS_originaldevdatawithmodelresults", replace
order *, sequential
...
lcolor (midblue gray gray black) lpat(solid solid solid dash) lwidth(*4 *1 *1 *1) legend(size(*0.75)) xlabel(0(500)3000) ylabel(0.0(.1)0.7)
...
lcolor (midblue gray gray black) lpat(solid solid solid dash) lwidth(*4 *1 *1 *1) legend(size(*0.75)) xlabel(0(500)3000) ylabel(0.0(.02)0.14)				
