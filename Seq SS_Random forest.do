...
* cd "C:\Users\User XYZ\Stata Results Tables_Random forest\"

cd "C:\Users\leghaa\OneDrive - University of Birmingham\Sequential SS\Updated and Simplified Results\Stata Results Tables_Random Forest_Github Test\"

* For BEAR use this WD:
* cd "/rds/projects/l/leghaa-seq-samp-size/NewSimplifiedApproach_RandomForest_May25GithubTest"
...
/* 0.21 Creating a new variable for creatinine mean by multiplying the original 
values by 1000 (as there was an issue where the rforest package errored when using 
small numbers, this workaround fixes the problem whilst simultaneously not 
changing the model - as the beta coefficient for creatinine_mean has only been 
scaled up) */

gen creatinine_1000 = creatinine_mean*1000

/* 0.3 Keep only the outcome (development of acute kidney injury) and the 6 predictors of interest */

keep AKI bicarbonate_mean creatinine_1000 hemoglobin_mean bun_mean potassium_mean sysbp_mean
...
/* 1.2 Apply the initial model which is the random forest model to the development dataset.

We are using iterations(100) = 100 trees; and type(class) indicates it is a 
classification problem (suitable for binary outcomes as in our study)
*/

qui rforest AKI bicarbonate_mean creatinine_1000 hemoglobin_mean ///
bun_mean potassium_mean sysbp_mean, type(class) iterations(100) depth(3)
...
use "RandomForest_originaldevdatawithmodelresults", clear
keep AKI bicarbonate_mean creatinine_1000 hemoglobin_mean bun_mean potassium_mean sysbp_mean
...
/* 3.1 Apply the initial model which is the random forest model to the development dataset.
...
use "RandomForest_originaldevdatawithmodelresults", replace
order *, sequential
...
lcolor (midblue gray gray black) lpat(solid solid solid dash) lwidth(*4 *1 *1 *1) legend(size(*0.75)) xlabel(0(500)3000) ylabel(0.0(.05)0.3)
